...
import { MdOutlineEmojiPeople } from 'react-icons/md' //人物招手
        ...
        >
          ...
          {stores.map((v, i) => (
            <AnyReactComponent
              ...
              lng={v.location.lng}
              text={v.sName}
              src={<FaMapMarkerAlt style={{ color: 'purple' }} size="2em" />}
            />
          ))}
          {peopleShow && (
            <AnyReactComponent
              ...
              lng={getGps.lng}
              text={''}
              src={<MdOutlineEmojiPeople style={{ color: 'red' }} size="5em" />}
            />
          )}
        </GoogleMapReact>
      </div>
...
export default withRouter(StoresMapForGoogle)
